...
func NewRaceProcessor(store Store, iracingClient IRacingClient) *RaceProcessor {
...
func (r *RaceProcessor) IngestRaces(ctx context.Context, request RaceIngestionRequest) error {
	...
	for _, race := range results {
		...
		for _, simSession := range sessionResult.SessionResults {
				...
				if insertSessionData {
					...
					for _, lap := range laps.Laps {
						insertions.SessionDriverLapEntries = append(insertions.SessionDriverLapEntries, store.SessionDriverLap{
							...
							LapTime:      time.Duration(lap.LapTime) * 100 * time.Microsecond,
							Flags:        lap.Flags,
			...
			// record progress so if things bomb out on subsequent races were not re-processing them
			err = r.store.UpdateDriverRacesIngestedTo(ctx, driver.DriverID, sessionResult.EndTime)
			if err != nil {
